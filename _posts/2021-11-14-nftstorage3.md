...
<html>
    <head>
        ...
        <p>
        <p>- ICN과 IPFS의 주요 차이점
        <p style= "text-indent: 10px">1. ICN/NDN은 종종 링크 계층에 있는 반면 IPFS는 응용 프로그램 계층에 있으며 링크 및 전송 계층에서 사용할 수 있는 모든 것을 활용한다.</p>
        <p style= "text-indent: 10px">2. 암호화 주소 지정은 ICN/NDN 스택의 핵심 원시적이지 않다. ICN/NDN은 전달된 데이터가 요청된 데이터임을 확인할 수 있는 방법이 없다(HTTP와 비슷하다).</p>
        <p style= "text-indent: 10px">3. ICN/NDN은 라우팅 설정 방법을 결정하는 제어 지점에 의존한다.</p>
        <p style= "text-indent: 10px">4. IPFS는 노드 이탈에 매우 탄력적이고 ICN/NDN 네트워크는 링크 계층에 너무 가깝기 때문에 그렇지 않다.</p>
        <p style= "text-indent: 10px">5. IPFS는 안전한 변경 가능한 포인터를 위해 SFS의 자체 인증 명명(SFS)을 사용한다. ICN/NDN은 이를 전파하기 위해 제어 지점에 의존한다.</p>
        <p style= "text-indent: 10px">6. 안타깝게도 ICN/NDN 세계는 공공 및 권한 없는 네트워크에 초점을 맞추지 않았기 때문에 검증 가능성과 신뢰할 수 없는 상호 작용보다는 제어 신뢰가 많은 설계이다.</p>        
        <p>
        ...
        <p style= "text-indent: 10px">1) 콘텐츠 주소 지정을 통한 고유 식별</p>
        <p style= "text-indent: 10px">2) 방향성 비순환 그래프(DAG)를 통한 콘텐츠 연결</p>
        <p style= "text-indent: 10px">3) DHT(분산 해시 테이블)를 통한 콘텐츠 검색</p>
        <br><br>
