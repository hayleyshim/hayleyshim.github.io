...
<html>
    <head>
        ...
        <br><br>
        <p>	
        <p><b>1. 콘텐트 주소 지정(Content Addressing)</b> : 사용자가 NFT.Storage에 데이터를 업로드하면 <a href="https://docs.ipfs.io/concepts/content-addressing/#identifier-formats">CID</a> 라고 하는 콘텐트의 IPFS 해시를 받는다 . CID는 데이터의 고유한 지문이며 저장 방법과 위치에 관계없이 콘텐트를 참조하는 데 사용할 수 있는 범용 주소이다. CID는 콘텐트 자체에서 생성되기 때문에 CID를 사용하여 NFT 데이터를 참조하면 취약한 링크 및 "러그 풀(rug pull)" 과 같은 문제를 방지할 수 있다 .
        ...
        <p>  
        <p><b>1. 콘텐트 주소 지정(Content Addressing)</b>
        ...
        <p>- ICN과 IPFS의 주요 차이점
        <blockquote>
        <p>1. ICN/NDN은 종종 링크 계층에 있는 반면 IPFS는 응용 프로그램 계층에 있으며 링크 및 전송 계층에서 사용할 수 있는 모든 것을 활용한다.</p> 
        <p>2. 암호화 주소 지정은 ICN/NDN 스택의 핵심 원시적이지 않다. ICN/NDN은 전달된 데이터가 요청된 데이터임을 확인할 수 있는 방법이 없다(HTTP와 비슷하다).</p> 
        <p>3. ICN/NDN은 라우팅 설정 방법을 결정하는 제어 지점에 의존한다.</p> 
        <p>4. IPFS는 노드 이탈에 매우 탄력적이고 ICN/NDN 네트워크는 링크 계층에 너무 가깝기 때문에 그렇지 않다.</p> 
        <p>5. IPFS는 안전한 변경 가능한 포인터를 위해 SFS의 자체 인증 명명(SFS)을 사용한다. ICN/NDN은 이를 전파하기 위해 제어 지점에 의존한다.</p> 
        <p>6. 안타깝게도 ICN/NDN 세계는 공공 및 권한 없는 네트워크에 초점을 맞추지 않았기 때문에 검증 가능성과 신뢰할 수 없는 상호 작용보다는 제어 신뢰가 많은 설계이다.</p>     
        </blockquote>
        ...
        <p>- 대부분의 NFT는 실제로 링크일 뿐이므로 Link rot은 NFT에 영향을 미친다. 보통 비용 문제로 NFT 내부에 오프체인 데이터에 대한 링크만 저장하기 때문에 접근할 수 없는 <a href="https://en.wikipedia.org/wiki/Link_rot">링크(Link rot)</a>와 함께 NFT를 저장하면 블록체인 기록이 변경되지 않은 상태로 유지되더라도 NFT의 가치가 손상된다.
        <p>- 따라서, NFT에서 오프체인 자산으로 안전하게 연결하려면 안전한 링크가 필요한데 콘텐트 주소 지정은 정확히 필요한 종류의 링크를 제공한다. 
        <p>- 콘텐트 주소 지정 시스템은 키-값 저장소처럼 작동하지만 한 가지 중요한 차이점이 있다. 더 이상 키를 선택할 수 없다는 것이다. 대신 키는 동일한 콘텐트에 대해 항상 동일한 키를 생성하는 결정적 함수를 사용하여 저장된 값에서 직접 파생된다.
        ...
        <p>- 콘텐트 주소 지정을 실제 활용하여 내구성 있는 링크가 있는 NFT를 만드는 방법이다.
            <p>- 가장 간단한 방법은 <a href="https://ipfs.io/">IPFS</a> 를 사용하는 것dl다. 데이터가 IPFS에 저장되면 사용자는 복사본이 있는 모든 IPFS 노드에서 데이터를 가져올 수 있으므로 데이터 전송을 보다 효율적으로 수행하고 단일 서버의 부하를 줄일 수 있다. 각 사용자는 데이터 조각을 가져올 때 나중에 요청할 수 있는 다른 사용자를 돕기 위해 로컬 복사본을 보관한다.
        ...
        <p>
        <br>  
        <p><font size="5"> 다음 편에서는 NFT Storage Service가 가져올 <font color="purple">   <b>비지니스 가치</b></font>에 대해 알아보자.
        <br>
